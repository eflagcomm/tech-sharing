...
- 监视 Spark 应用:
    - 启动历史日志服务器, `sbin/start-history-server.sh hdfs://eflagcluster/sparkeventlog`. 如果不带参数(由 conf/spark-defaults.conf 设置), 需要设置环境变量 [SPARK_HISTORY_OPTS](http://spark.apache.org/docs/latest/monitoring.html#environment-variables), 默认为空.
